...
@localio2001adjustments broached an influential overview of
...
We observe that across `r nrow(sim_summary) / 3` scenarios
and $R = 1{,}500$ replications per scenario, the random-site
...
[-@morris2019using]. The full audit is recorded at
...
- `R = 1500` is fixed without a MCSE-based derivation.
- `filter(!is.na(est))` silently drops non-converged
  replications, breaking paired comparisons.

A remediation plan is recorded in the audit file and
will be taken up in a subsequent revision.
