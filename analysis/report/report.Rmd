...
(Morris Table 6 Monte Carlo SEs for every metric in Table 1 are
given in Table 2; the reported point estimates in Table 1 are
unrounded relative to those MCSEs). The fixed-site estimator closely
...
the null-hypothesis scenarios ($\delta = 0$; Table 3), the empirical
...
the boundary/singular fits flagged in Table 1 and Table 3 return a
...
"Analytic methods"); the null-arm results in Table 3 show
...
4. **Small-sample corrections.** The current implementation
   ...
   null-arm scenarios simulated here (Table 3) but is not a
