...
\renewcommand{\stamptime}{2026-08-13 12:34 PDT}
\renewcommand{\stampversion}{\detokenize{e586e83-wip}}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{lineno}
\linenumbers
...
absolute bias never exceeding 0.011 (Morris Table 6 Monte Carlo SEs for
every metric in Table 1 are given in Table 2; the reported point
estimates in Table 1 are unrounded relative to those MCSEs). The
fixed-site estimator closely tracked the random-site estimator. Ignoring
site entirely produced overcoverage (up to 0.984) and correspondingly
lower power (as low as 0.40) in scenarios with non-trivial between-site
variance. The lowest convergence rate (hard \texttt{lmer} errors only)
observed across any scenario or method was 1.000; however, the
random-site model's \emph{singular/boundary} fit rate reached as high as
0.557 in the low- and no-site-variance scenarios, where the true
random-intercept variance is at or near the parameter-space boundary. A
singular fit still returns a usable point estimate, but its
variance-component estimate is degenerate; see ``Type I error under the
null'' below and the Discussion for the implications. Under the
null-hypothesis scenarios (\(\delta = 0\); Table 3), the empirical Type
I error rate for the random-site method ranged from 0.041 to 0.068, and
for the ignore-site method from 0.014 to 0.047, against a nominal
\(\alpha = 0.05\).
...
Table 3 reports the empirical rejection rate at \(\alpha = 0.05\) for
...
general; the boundary/singular fits flagged in Table 1 and Table 3
...
methods''); the null-arm results in Table 3 show this approximation was
...
\item
  ...
  3) but is not a general substitute for a proper small-sample
...
\emph{Rendered on 2026-08-13 at 12:58 PDT.}\\
\emph{Source:
