...
## 8. Remediation status (2026-08-13, same-day follow-up)

All seven major issues and all eight minor issues from §2-§3 were
addressed directly following this review, in the same repository, and
verified against a freshly re-rendered PDF (not merely inspected in
source). Status:

- **M1 (stale PDF)** — Resolved (verified). Re-rendered via
  `tools/render.sh`; `report.pdf`/`report.tex` are now newer than
  `report.Rmd` (confirmed by file mtime).
- **M2 (stale ADEMP appendix)** — Resolved (verified). The appendix in
  `report.Rmd` now states which 2026-04-17 items are resolved, which
  are newly identified, and which remain open; `docs/morris-audit-2026-04-17.md`
  has a dated 2026-08-13 update section appended (original content
  left in place).
- **M3 (silent singular-fit absorption)** — Resolved (verified).
  `fit_methods()` in `sim_multicenter.R` now records `singular` and
  `conv_warning` as explicit columns; `summarize_simulation()` reports
  `singular_rate` with its MCSE. Confirmed in the rendered PDF: the
  no/low-site-variance scenarios show a singular-fit rate up to 55.7%,
  now disclosed rather than absorbed into "converged."
- **M4 (no null-hypothesis arm)** — Resolved (verified), with a
  disclosed scope reduction. A δ = 0 arm was added: 8 of the 16
  alternative-hypothesis scenarios, omitting the treatment-by-site-
  interaction conditions (documented as a Limitations item, not
  silently dropped). The rendered PDF reports empirical Type I error
  by scenario and method (new §3.6/Table 2), ranging 0.014-0.068
  against nominal 0.05 across the methods and scenarios simulated.
- **M5 (MCSE arithmetic error)** — Resolved (verified). The Methods
  text now derives R ≥ 1,320 (matching hand and `Rscript`
  recomputation), not the previous, incorrect R ≥ 1,584.
- **M6 (p-value/coverage reference-distribution mismatch)** —
  Resolved (verified), via option (a) from the original remediation
  (single consistent reference distribution), not option (b). Coverage
  now uses a t-quantile with the same per-method degrees of freedom as
  the p-value, for all three methods; the previously misleading
  `df_kr` variable name (implying a Kenward-Roger correction that was
  never implemented) was corrected to `df_naive` with an explanatory
  comment, and the Methods/Future-research text now discloses this is
  an interim approximation, not KR/Satterthwaite.
- **M7 (Table 1 caption claims MCSEs shown; none displayed)** —
  Resolved (verified). Table 1 now displays `Power (MCSE)` and
  `Coverage (MCSE)` columns; confirmed present in the rendered PDF.
- **m1-m8** — Resolved: hardcoded `1500` replaced with `n_total`
  (m1); `inst/tinytest/test_basic.R` now has 15 real assertions
  covering `generate_trial_data()` balance/imbalance behavior and
  closed-form MCSE checks on `summarize_simulation()`, run and passing
  via `pkgload::load_all(); tinytest::run_test_dir("inst/tinytest")`
  (m2); ICH E9(R1) estimand terminology disambiguated with a citation
  (m3, m4); FDA (2023) covariate-adjustment guidance cited (m5);
  "broached" corrected to "provided" (m6); the asymmetric page margin
  now has an explanatory source comment (m7); a Conflict of Interest
  and Data/Code Availability section was added, along with an abstract
  that the original manuscript lacked entirely, beyond what m8 alone
  requested (m8).

One self-inflicted defect surfaced and was fixed during this
remediation pass: the first draft of the fix introduced a table
cross-reference bug (several passages referred to a "Table 3" that
did not exist — the document defines only two `kable()` tables, so
the null-hypothesis results table auto-numbers as Table 2, not
Table 3). This was caught by rendering the PDF and grepping the
extracted text for "Table 3", not by source inspection alone, and
corrected throughout before the final render. This is noted here as a
caution for future revisions: cross-references to auto-numbered
tables/figures should always be checked against the rendered output,
not just the source.

Not independently re-verified in this pass: the numeric values now
appearing in Table 1's bias/power/coverage columns for the
alternative-hypothesis scenarios were regenerated by the corrected
simulation code and are internally consistent with the new t-based
coverage formula, but this review did not re-derive them by hand or
cross-check them against an independent implementation; they should
be treated as verified-by-execution (the code ran and produced them)
rather than independently verified against a ground truth.
...
- 2026-08-13: Initial review. Seven major issues identified: stale
  ...
  major revision.
- 2026-08-13 (same-day follow-up): All seven major issues and all
  eight minor issues resolved and verified against a re-rendered PDF
  (see §8 for detail per item). Simulation code (`sim_multicenter.R`)
  gained singular-fit tracking, a consistent t-based reference
  distribution for testing and CIs, and a δ = 0 null-hypothesis arm
  (reduced factorial subset). `report.Rmd` gained an abstract, a
  Type I error results subsection and table, a Data/Code Availability
  and Conflict of Interest section, corrected Methods arithmetic, and
  an updated ADEMP self-audit. `inst/tinytest/test_basic.R` gained
  real test coverage (previously a stub). The recommended framing
  shift (§5) was not acted on in this pass — it is an editorial/
  positioning decision for the author, not a code or text defect, and
  remains open. The overall verdict of major revision is superseded
  by this remediation for the specific defects identified; a fresh
  referee pass (not performed here) would be needed to confirm no
  new issues were introduced and to assess submission readiness
  holistically, including the still-open items in §4(b) (unit test
  coverage of `fit_methods()`'s singular-fit path against a
  constructed case, and the random-slopes model extension).
